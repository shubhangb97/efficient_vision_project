...
def get_fid_metric(pred1,target1):
    ...
    # smpl = None
    # print (pred1.shape)
    pred1 = pred1.reshape(pred1.shape[0],pred1.shape[1],-1).detach().numpy()
    # print (pred1.shape)
    target1 = target1.reshape(target1.shape[0],target1.shape[1],-1).detach().numpy()
    ...
    for i in range(pred1.shape[0]):
        try:
            temp1 = extract_feature(target1[i,:,:].reshape(1,target1.shape[1],225), smpl, "kinetic")
            temp2 = extract_feature(target1[i,:,:].reshape(1,target1.shape[1],225), smpl, "manual")
            temp3 = extract_feature(pred1[i,:,:].reshape(1,pred1.shape[1],225), smpl, "kinetic")
            temp4 = extract_feature(pred1[i,:,:].reshape(1,pred1.shape[1],225), smpl, "manual")
            gt_kin.append(temp1)
            gt_man.append(temp2)
            res_kin.append(temp3)
            res_man.append(temp4)
        except:
            pass
